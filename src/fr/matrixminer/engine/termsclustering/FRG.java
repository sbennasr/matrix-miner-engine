...
public class FRG extends JPanel {
	...
	public FRG(Map<FeatureCouple, Double> similarityMap,
			CustomStringList valFeatures, File directory) throws IOException,
		...
		for (String feature : valFeatures) {
			...
			lemmaValFeatures.put(feature, lemmaFeature);
		...
		generateCsvFile(valPath+"featureTrans.csv");
		fileName = "cluster1";
		tagFile = new File(dir, fileName + ".csv");
		if (!tagFile.exists()) {
			tagFile.createNewFile();
		}
		generateItemsetsCsvFile(valPath+"cluster1.csv",featureTrans);
