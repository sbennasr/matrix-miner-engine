...
public class MatrixMinerTest {
	...
	public static void main(String args[]) throws Exception {
		directory = new File(
				"C:\\Users\\sbennasr\\FinalSpace\\MatrixMiner Engine\\products\\Cell phones");
		dir = directory;
		...
		// reducePCM();
		/*
		 * featuresBoolDiff(); refactorBoolPCM(); generateBoolResultingPCM();
				...
				if (!areDiffRows(tempMap)) {
					...
					for (Point point : points) {
						...
						if (cellVal.equals("") && !newCellVal.equals("")) {
							...
							System.out
									.println("remainingList:" + remainingList);
						}
					}
				...
				for (String feature : cluster) {
					if (!feature.equals(commonItem)) {
						...
						while (textValItPath.hasNext()) {
							...
							System.out.println(textValFeatureList);
							System.out.println("lemma Text val list:"
									+ textValLemmaFeatureList);
				...
				for (String feature : cluster) {
					if (!feature.equals(commonItem)) {
						...
						while (valItPath.hasNext()) {
							...
							System.out.println("lemma val list:"
									+ valLemmaFeatureList);
		...
		if (!areDiffRows(tempMap)) {
			...
			for (Point point : points) {
				...
				if (cellVal.equals("") && !newCellVal.equals("")) {
					...
					featuresMap.put(path, tempFeatures);
					List<String> remainingList = featuresMap.get(path);
					System.out.println("remainingList:" + remainingList);
				}
			}
			return false;
		}
	...
	private static void featuresTextValDiff(File dir)
		...
		System.out.println("Textual Quantified features:" + textValFeatures
				+ "\n");
		TextValFeatureClustering clusFeat1 = new TextValFeatureClustering(
				directory);
	...
	private static void featuresValDiff(File dir)
			throws ClassNotFoundException, IOException, InterruptedException {
	...
	private static void featuresTextDiff(File dir)
			throws ClassNotFoundException, IOException, InterruptedException {
	...
	private static void featuresBoolDiff() throws ClassNotFoundException,
		...
		System.out.println("Bool features:" + candidBoolList + "\n");
		...
		System.out.println("bool features:" + boolFeatures + "\n");
				...
				for (String feature : cluster) {
					if (!feature.equals(commonItem)) {
						...
						while (boolItPath.hasNext()) {
							...
							System.out.println(boolFeatureList);
	...
	public static void seekFeaturesByFile(File dir) throws Exception {
		...
		for (int i = 0; i < plFiles.size(); i++) {
			...
			System.out.println("Textual Val features Map: " + candidTextValMap);
		}
		System.out.println("Quantified features Map: " + candidValMap);
		System.out.println("Textual features Map: " + candidTextMap);
		System.out.println("Textual Val features Map: " + candidTextValMap);
